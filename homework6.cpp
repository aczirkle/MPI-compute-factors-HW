/** 
    ...
    Copyright (C) 2015 zirkleac@miamiOH.edu
 */
...
void doManagerTasks(const std::string& path) {
    ...
    //std::cout<< "Worker Size"<< npes<<endl;
    if (!data.good()) {
  ...
  MPI_Send(&number, 1, MPI_LONG_LONG_INT, i, NUMBER_TAG, MPI_COMM_WORLD);
            }

            for (int i = 1; i < npes; i++) {
         MPI_Recv(&factors, 1, MPI_INT, i, FACTORS_TAG, MPI_COMM_WORLD, NULL);
                // Print results
